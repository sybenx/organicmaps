...
final class ProductsManager {
  ...
  fileprivate enum ProductsVisibilityCondition {
    case promotionClosedTimeoutExpired(closedTime: TimeInterval)
  ...
  func fetchProductsConfiguration(for screen: ProductsScreen) throws -> ProductsConfiguration? {
    ...
    guard let configString = settings.getProductsConfiguration() else {
      throw ProductError.missedProductsConfig
    }
  ...
  private func shouldShowProducts(for screen: ProductsScreen) -> Bool {
    ...
    case .placePage:
      conditions = [
        .promotionClosedTimeoutExpired(closedTime: settings.productsDialogClosedTime()),
...
private extension ProductsManager.ProductsVisibilityCondition {
  static let kMinTimeoutSinceLastClosed: TimeInterval = {
    #if DEBUG
    return 10 // 10 sec
    #else
  ...
  static let kMinTimeoutSinceFirstLaunch: TimeInterval = {
    #if DEBUG
    return 5 * 60 // 5 min - usually it may takes some time to download the map after the first launch
    #else
  ...
  func evaluate() -> Bool {
    switch self {
    case .promotionClosedTimeoutExpired(let closedTime):
      if closedTime == 0 {
        return true // The dialog was never closed.
      }
      return closedTime + Self.kMinTimeoutSinceLastClosed < Date().timeIntervalSince1970
...
extension ProductError: LocalizedError {
  var errorDescription: String? {
    ...
    case .missedProductsConfig:
      return "Missed products configuration"
    case .failedToDecodeConfig:
